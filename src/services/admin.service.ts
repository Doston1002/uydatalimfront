...
export const AdminService = {
  ...
  async changeUserRole(userId: string, role: "ADMIN" | "INSTRUCTOR" | "USER") {
    const { data } = await $axios.put<{ message: string; user: UserType }>(
      `${getAdminUrl("update-user-role")}`,
      {
        userId,
        role,
      },
